...
# setup fake scripts
# knot-fog
COPY ./scripts/docker-knot-service.sh /usr/local/bin/knot-fog
RUN sed -i 's/knotctl/knot-fog/g' /usr/local/bin/knot-fog
# knot-web
COPY ./scripts/docker-knot-service.sh /usr/local/bin/knot-web
RUN sed -i 's/knotctl/knot-web/g' /usr/local/bin/knot-web
# knotd
COPY ./scripts/docker-knot-service.sh /usr/local/bin/knotd
RUN sed -i 's/knotctl/knotd/g' /usr/local/bin/knotd
# nrfd
COPY ./scripts/docker-knot-service.sh /usr/local/bin/nrfd
RUN sed -i 's/knotctl/nrfd/g' /usr/local/bin/nrfd
# lorad
COPY ./scripts/docker-knot-service.sh /usr/local/bin/lorad
RUN sed -i 's/knotctl/lorad/g' /usr/local/bin/lorad
