...
createTest(configDataWithState)('getState() returns the state from the file', async (t, settingsStore) => {
  const state = await settingsStore.getState();
...
createTest(configDataWithStateCloud)('getCloud() returns cloud configuration as Address', async (t, settingsStore) => {
...
createTest(configDataWithStateCloud)('getCloud() returns the configuration from the file', async (t, settingsStore) => {
  const configuration = await settingsStore.getCloud();
...
createTest(configDataWithState)('getCloud() returns null if the file doesn\'t contain the cloud object', async (t, settingsStore) => {
  const configuration = await settingsStore.getCloud();
...
createTest(configDataWithState)('setCloud() writes cloud object on the file', async (t, settingsStore) => {
...
createTest(configDataWithState)('setCloud() writes cloud object on the file with the address contents', async (t, settingsStore) => {
  const address = new Address('localhost', 3000);
  await settingsStore.setCloud(address);
...
createTest(configDataWithStateCloud)('setCloud() update the cloud object if it exists', async (t, settingsStore) => {
